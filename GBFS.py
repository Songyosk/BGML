...
class GBFS():
    ...
    args: 
        ...
        (5) oversampled_it (type:bool) - whether to oversampled the training data; choose False if already oversampled
        (6) problem (type:str) - whether it is a 'classification' or 'regression' problem
        (7*) target_classes (type:int) - for classification, specify the number of target classes

    return: 
        (1) list of features selected during GBFS
    """

    def __init__(self, path_to_file, path_to_save, target, features, oversampled_it, problem, *args, **kwargs):
    ...
    def oversample(self, df, technique, *args, **kwargs):
        ...
        if technique == 'ros':
            ...
            os = RandomOverSampler(shrinkage=0.35)
        ...
        # Add column names
        oversampled_train.columns = df[self.features + [self.target]].columns
            ...
            for fold in range(cv_folds):
                ...
                if self.problem == 'classification':
                    if self.oversampled_it == True:
                        ...
                        if fold == 0:
                            print('   No. of rows in training set after oversampling:', len(oversampled_tr))
                    ...
                    try:
                        if self.objective == 'multiclass' or self.objective == 'multi:softprob':
                            score = roc_auc_score(
    ...
    def run(self, boosting_method, cv_folds, oversample_technique='smoothed_ros', *args, **kwargs):
        """
        ...
        args: 
            ...
            (2) objective (type:str) - For classification, multiclass or binary ('multiclass', 'multi:softprob', 'binary' etc)
        ...
        joblib.dump(self.saved_model, os.path.join(self.path_to_save, r'model_from_GBFS_' + str(self.target) + '.pkl'))
        
        print('\n', 'Model saved as:', 'model_from_GBFS_' + str(self.target) + '.pkl')

        return self.saved_model



    def feature_relevance(self, plot, no_of_features, log_scale=True):
        """
        ...
        #Save data
        joblib.dump(self.feature_score, os.path.join(self.path_to_save, r'feature_relevance_score_' + str(self.target) + '.pkl'))

        print('Result saved as: feature_relevance_score_' + str(self.target) + '.pkl')
        ...
        if plot == True:
            ...
            plt.savefig(os.path.join(self.path_to_save, r'feature_relevance_plot_' + str(self.target) + '.png'), dpi = 300)

            print('Figure saved as: feature_relevance_plot_' + str(self.target) + '.png')
    ...
    def recursive_selection(self, stratify, oversample_technique, chosen_metric, average, no_to_terminate, max_no_imp, threshold=0.5):
        """
        ...
        Args: 
            ...
            (7) threshold (type:float) - the change in performance metric to consider it as an improvement
        ...
        if self.problem == 'classification' and stratify == True:
            sample_tr, sample_va = train_test_split(self.sample_train, test_size = 0.20, stratify = self.sample_train[self.target].to_list())

        else:
            sample_tr, sample_va = train_test_split(self.sample_train, test_size=0.20)
        ...
        for n in n_range:
            ...
            if self.problem == 'classification':
                ...
                if self.objective == 'binary':
                    train_roc_score = roc_auc_score(y_train, y_train_pred[:,1], average = average)
                    
                    train_avg_p_score = average_precision_score(y_train_bin, y_train_pred[:,1], average = average)
                ...
                if self.objective == 'binary':
                    va_roc_score = roc_auc_score(y_va, y_va_pred[:,1],  average = average)
                    va_avg_p_score= average_precision_score(y_va_bin, y_va_pred[:,1], average = average)
                else:
                ...
                if n > 2:
                    ...
                    sig = 4
                    metric_change = abs(round(metric[-2], sig) - round(metric[-1], sig))
                    ...
                    print('Metric change:', metric_change)
                    print()
            ...
            if self.problem == 'regression':
                ...
                if n > 2:
                    ...
                    if chosen_metric == 'mae':
                        ...
                        # if round(metric[-2], sig) <= round(metric[-1], sig):
                        #     no_imp = no_imp + 1
                        # else:
                        #     no_imp = 0
            ...
            if self.problem == 'classification':
                self.result = pd.DataFrame(data = list(zip(n_range, 
                                                           tr_ac, va_ac,
                                                           tr_b_ac, va_b_ac,
                                                           tr_hl, va_hl,
                                                           tr_pr, va_pr,
                ...
                self.result.columns = ['no_of_features', 
        ...
        joblib.dump(self.result, os.path.join(self.path_to_save, r'GBFS_result_' + str(self.target) + '.pkl'))

        print('Result saved as: GBFS_result_' + str(self.target) + '.pkl')
    ...
    def convergence_plot(self, *args, **kwargs):
        ...
        plt.savefig(os.path.join(self.path_to_save, r'GBFS_convergence_plot_' + str(self.target) + '.png'), dpi = 300)

        print('Result saved as: GBFS_convergence_plot_' + str(self.target) + '.png')
